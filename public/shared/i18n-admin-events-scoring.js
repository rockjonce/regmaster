...
(function () {
  ...
  var Z = {
    ...
    aeScoreSubmitToast: '已送出評分',
    aeScoreRankBasisNote: '總名次以「原始加權分」排序（跨組可比）；組內名次以組內正規化分排序。未評分／備取不列入排名。',
    aeScoreResTotalRaw: '原始加權分',
    aeScoreResTotalNorm: '總分（組內正規化）',
    aeScoreClampedToast: '已超過滿分，自動調整為 {max} 分',
    aeScoreMaxLabel: '滿分',
    aeScoreRecordsLabel: '可記錄次數',
    ...
    aeScoreJudgeBoxTitle: '盒鬚圖（各組別評審間評分分布）',
    aeScoreJudgeBoxHint: '每筆為一位評審對該組某隊的原始加權總分（0–100），用以判斷各組評審評分的區間與一致性。',
  ...
  var E = {
    ...
    aeScoreSubmitToast: 'Scores submitted',
    aeScoreRankBasisNote: 'Overall ranks use the raw weighted score (comparable across groups); in-group ranks use normalized scores. Unscored / waitlisted teams are not ranked.',
    aeScoreResTotalRaw: 'Raw weighted score',
    aeScoreResTotalNorm: 'Total (normalized in group)',
    aeScoreClampedToast: 'Above the maximum — adjusted to {max}',
    aeScoreMaxLabel: 'Max',
    aeScoreRecordsLabel: 'Recordable count',
    ...
    aeScoreJudgeBoxTitle: 'Box-and-whisker plot (inter-judge score spread per group)',
    aeScoreJudgeBoxHint: 'Each point is one judge’s raw weighted total (0–100) for a team in the group — use it to gauge each group’s judge score range and consistency.',
