...
(function () {
  ...
  Object.assign(Z, {
    ...
    aePayRefundReqWarn: '⚠️ 此報名者已提出退費申請並待處理。建議改用「平台代收轉付 → 退刷」依退費政策處理，以免退款金額與政策不一致。',
    aePayRefundKeepNote: '此操作會退費但「保留報名」（不釋出名額）。若要退費並取消報名／釋出名額，請改用「平台代收轉付 → 退刷」。',
  ...
  Object.assign(E, {
    ...
    aePayRefundReqWarn: '⚠️ This registrant has submitted a refund request that is pending. Consider using "Platform payout → Refund" to process it per the refund policy, so the amount stays consistent.',
    aePayRefundKeepNote: 'This refunds but KEEPS the registration (the spot is not released). To refund and cancel the registration / release the spot, use "Platform payout → Refund".',
